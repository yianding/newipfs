...
func GetLocalIPv6() (net.IP, error) {
	conn, err := net.Dial("udp", "[2600::]:80")
	if err != nil {
		//	log.Errorw("GETIPV6", err)
		return nil, err
	}
	defer conn.Close()
...
func (h *BasicHost) updateLocalIpAddr() {
	...
	//log.Errorw("ANDY get ip address for libp2p")
	...
	if err != nil {
		...
		ipv4, _ := GetLocalIPv4()
		ipv4addr, err := manet.FromIP(ipv4)
		if err == nil {
			fmt.Println("GETIPV4 ", ipv4addr.String())
			h.filteredInterfaceAddrs = append(h.filteredInterfaceAddrs, ipv4addr)
		}

		ipv6, _ := GetLocalIPv6()
		ipv6addr, err := manet.FromIP(ipv6)
		if err == nil {
			fmt.Println("GETIPV6", ipv6addr.String())
			h.filteredInterfaceAddrs = append(h.filteredInterfaceAddrs, ipv6addr)
		}
		...
		fmt.Println("ANDY IP11", h.allInterfaceAddrs, h.filteredInterfaceAddrs)
		return
	}
	...
	fmt.Println("ANDY IP12", h.allInterfaceAddrs, h.filteredInterfaceAddrs)
}
